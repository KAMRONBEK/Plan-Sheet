...
const Checkout = ({navigation, item, modalOn}) => {
    ...
    if (Object.keys(state).length > 0) {
        Object.keys(state).forEach(key => {
            let el = state[key];
            totalCount += el.qty;
            totalPrice = el.qty * data.getProductById.price;
        });
    }
            ...
            <View style={styles.header}>
                <View style={styles.imageWrapper}>
                    <Image
                        ...
                        source={{
                            uri: data && data.getProductById.images && data.getProductById.images[0],
                        }}
                    />
                </View>
                <View style={styles.textWrapper}>
                    <Text style={styles.title}>{data && data.getProductById.name}</Text>
                    <Text style={styles.text}>
                        {strings.minimumOrder} {data && data.getProductById.min_order}
                    </Text>
                    <Text style={styles.price}>
                        {data && data.getProductById.price} {strings.priceUnit}
                    </Text>
                </View>
                <View
                    style={{
